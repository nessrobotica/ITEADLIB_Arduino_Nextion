...
bool ESPNexUpload::_searchBaudrate(uint32_t baudrate) {
  ...
  if (response[0] != 0x1A) {
    ...
    _printInfoLine(
        F("first respone from display, first indication that baudrate is "
          "correct"));
  }
  ...
  response = String("");
  delay(110);  // based on serial analyser from Nextion editor V0.58 to Nextion
               // display NX4024T032_011R
...
uint16_t ESPNexUpload::recvRetString(String &response, uint32_t timeout,
  ...
  if (ff_flag)
    response =
        response.substring(0, response.length() - 3);  // Remove last 3 0xFF
...
bool ESPNexUpload::_setPrepareForFirmwareUpdate(uint32_t upload_baudrate) {
  ...
  // Without flush, the whmi command will NOT transmitted by the ESP in the
  // current baudrate because switching to another baudrate (nexSerialBegin
  // command) has an higher prio. The ESP will first jump to the new
  // 'upload_baudrate' and than process the serial 'transmit buffer' The flush
...
bool ESPNexUpload::_echoTest(String input) {
  ...
  cmd += "\"";
  cmd += ",";
  cmd += String(input.length() + 1);  // removing 'p'
...
bool ESPNexUpload::_handlingSleepAndDim(void) {
  ...
  if (set_sleep) {
    cmd = F("sleep=0");
    this->sendCommand(cmd.c_str());
    // Unfortunately the display doesn't send any respone on the wake up request
    // (sleep=0) Let the ESP wait for one second, this is based on serial
    // analyser from Nextion editor V0.58 to Nextion display NX4024T032_011R
...
uint32_t ESPNexUpload::calculateTransmissionTimeMs(String message) {
  ...
  // To calculate how much microsecond one BIT of data takes with a certain
  // baudrate you have to divide the baudrate by one second. For example 9600
  // baud = 1000 000 us / 9600 ≈ 104 us The time to transmit one DATA byte (if
  // we use default UART modulation) takes 10 bits. 8 DATA bits and one START
  // and one STOP bit makes 10 bits. In this example (9600 baud) a byte will
  // take 1041 us to send or receive. Multiply this value by the length of the
  // message (number of bytes) and the total transmit/ receive time is
  // calculated.

  uint32_t duration_one_byte_us =
      10000000 / _baudrate;                     // 1000 000 * 10 bits / baudrate
  ...
  _printInfoLine("calculated transmission time: " + String(return_value_ms) +
                 " ms");
...
void ESPNexUpload::_printInfoLine(String line) {
  ...
  if (line.length() != 0) dbSerialPrintln(line);
}
